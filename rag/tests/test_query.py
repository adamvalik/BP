...
queries_and_expected = [
    ...
    ("what skills can I learn to start a career in graphic design?", "graphics_9.txt"),
    ...
    ("were there any snacks designed for astronauts?", "space_62.txt"),
...
def test_query_hybrid_exact(rag_pipeline, query, expected_filename):
    ...
    if objects:
        actual_filename = objects[0].properties["filename"]
        assert actual_filename == expected_filename, f"Top-1 Mismatch for '{query}': Expected '{expected_filename}' but got '{actual_filename}'"
    else:
        assert expected_filename == "No results found", f"Expected 'No results found' for '{query}' but got results."
